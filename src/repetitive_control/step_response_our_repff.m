...
% Nominal Data for Nonlinear Parameters
...
% % F Block Design 1
% Fzpet_num = TF_yr.Denominator{1};
% Fzpet_dem = [TF_yr.Numerator{1}(2:end) 0]; 
% N1 = 1;   %F*z^-N1

% F Block Design 2
F = z*(1-0.6871*z^-1)*(1-1.962*z^-1+0.9635*z^-2)*(1+0.9993*z)/...
    (0.0064538*(1-0.9692*z^-1)*(1+0.9993)^2); % Given by Inverse TF_yr
% F = z*(1-0.6871*z^-1)*(1-1.962*z^-1+0.9635*z^-2)*(1+z)/...
%     (0.0064538*(1-0.9692*z^-1)*(1+1)^2); % Approximation 0.9993->1
Fzpet_num = F.Numerator{1};
Fzpet_dem = [F.Denominator{1}(3:end) 0 0]; 
...
Qzpet_dem = [Q.Denominator{1}(mm+1:end) zeros(1, mm)];
N2 = mm;   %Q*z^-N2 to be causal

% Signal Type
Signal_all = ["sine","rect","trig"];
sig = Signal_all(3); % Selct different mode to present different task
switch(sig)
    case {'sine'}
        Np = 0.5;
    case {'rect'}
...
%             '277_Assignments_Xiaochen\data\our_model_data\our_inverted_pend\'...
%             'inverted_pendulum_SOFC_',convertStringsToChars(method),'_response_',num2str(1/Ts),'Hz.mat'],'out')
%     case{'SOFCI'}
%         open('SOFCI_integral_simulation')
%         out = sim('SOFCI_integral_simulation');
%         save(['F:\！！！UCLA学习与课程\#####2023 Winter Quarter\' ...
%             '277 - Advanced Digital Control for Mechatronic Systems\' ...
%             '277_Assignments_Xiaochen\data\our_model_data\our_inverted_pend\'...
%             'inverted_pendulum_SOFCI_',convertStringsToChars(method),'_response_',num2str(1/Ts),'Hz.mat'],'out')
%     case{'SOFCIO'}
%         open('SOFCI_integral_simulation')
%         out = sim('SOFCI_integral_simulation');
%         save(['F:\！！！UCLA学习与课程\#####2023 Winter Quarter\' ...
%             '277 - Advanced Digital Control for Mechatronic Systems\' ...
%             '277_Assignments_Xiaochen\data\our_model_data\our_inverted_pend\' ...
%             'inverted_pendulum_SOFCIO_',convertStringsToChars(method),'_response_',num2str(1/Ts),'Hz.mat'],'out')
% end
