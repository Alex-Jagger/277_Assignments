...
pend = 2; %1: single rotot-pendulum, 2: double rotor-pendulum
...
switch(pend)
    ...
    case{2}
        % Double pendulum
        Kc = 1;          % Equivalent torsional pring constant Nm/radian
        A = [0 1; -K_tot/J_pend -1/tau];
        B = [0; kapa/tau];
        C = [1 0];
        Bd = [0; 1/J_pend];
   
        A_double = [A-Kc*Bd*C Kc*Bd*C; Kc*Bd*C A-Kc*Bd*C];
        B_double = [zeros(2, 1); B];
        C_double = [C zeros(1, 2)];
...
zpk(G_d)
        ...
        switch(pend)
            ...
            case{2}
               %Bw = F_rotorred*B_d;    % Double pendulum disturbance torque on x2
               Bw = F_rotorred*[B_d(3:4);B_d(1:2)];    % Double pendulum disturbance torque on x1
        ...
        if pend ~= 1; 
            ...
            Wn2 = 6*Wn;        
        s_pole2= [(-Zeta2+sqrt(Zeta2^2-1))*Wn2,  (-Zeta2-sqrt(Zeta2^2-1))*Wn2];
        Pole_SF2=exp(s_pole2*Ts);
       
            Pole_SF = [Pole_SF Pole_SF2]; end
